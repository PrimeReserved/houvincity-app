    ...
    return (
      <div className="mt-5">
        <div className="md:flex gap-5 space-y-5 md:space-y-0">
          <h1 className="font-semibold text-lg mt-1">Blog Post not available</h1>
        </div>
      </div>
    );
  }
      ...
      <div className="md:flex gap-5 space-y-5 md:space-y-0">
        <div className="card lg:w-[400px] lg:h-[580px] bg-base-100 drop-shadow-md">
          <div className="mt-3.5 ml-2.5">
            {article?.mainImage && (
               <img src={urlFor(article?.mainImage?.asset?._ref).width(380).height(500).url()} alt={article?.mainImage?.alt} />
            )}
          </div>
